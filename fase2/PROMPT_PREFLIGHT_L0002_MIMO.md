...
Realiza exclusivamente el preflight determinista de todos los documentos del manifiesto `corpus/lotes/L0002.json`. No hagas enriquecimiento, no leas analíticamente los textos, no construyas JSON analíticos, no ejecutes `ledger.py` y no escribas en `corpus/resultados/`.
...
4. Si el endpoint es apto y tiene saltos de página, registra `endpoint_apto`; no descargues PDF ni uses OCR.
5. Si falla, descarga el PDF, ejecuta `preflight_pdf.py` hacia `tramos/`; solo si devuelve `2`, aplica `ocrmypdf --force-ocr --deskew --language spa` y repite hacia `tramos_force_ocr/`.
