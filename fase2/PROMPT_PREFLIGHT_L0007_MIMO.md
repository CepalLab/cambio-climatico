...
`corpus/lotes/L0007.json`. No hagas enriquecimiento, no leas analíticamente los textos, no construyas
JSON analíticos, no ejecutes `ledger.py` y no escribas en `corpus/resultados/`.

El lote L0007 ya fue reservado en el ledger. Antes de procesar cada documento, verifica sus artefactos
...
6. Solo si `preflight_pdf.py` devuelve código `2`, crea una copia OCR con
...
7. Escribe `PREPARACION_FUENTE.md` dentro de `corpus/intermedios/11362/<id>/` con fuente seleccionada,
   métricas, rutas, necesidad de muestra visual y bloqueos. No declares revisión visual realizada si no fue
   efectivamente hecha.
...
(`endpoint_apto`/`pdf_tramos`/`ocr_forzada`/`bloqueado`), manifest o ruta de tramos, y si requiere revisión
visual.
...
El manifiesto reservado es `corpus/lotes/L0007.json` y contiene 15 documentos. La misión termina al completar
