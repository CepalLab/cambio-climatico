...
`corpus/lotes/L0009.json`. No hagas enriquecimiento, no leas analíticamente los textos, no construyas
JSON analíticos, no ejecutes `ledger.py` y no escribas en `corpus/resultados/`.

El lote L0009 ya fue reservado en el ledger. Antes de procesar cada documento, verifica sus artefactos
...
9. En cada bitácora o reporte de ejecución deja constancia del harness/modelo usado para esta misión:
   `Gemini CLI / Gemini 3.7 Flash` (el preflight sigue siendo determinista y local; no ejecutes
   enriquecimiento).
...
El manifiesto reservado es `corpus/lotes/L0009.json` y contiene 15 documentos. La misión termina al completar
