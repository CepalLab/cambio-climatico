...
`corpus/lotes/L0010.json`. No hagas enriquecimiento, no leas analíticamente los textos, no construyas
JSON analíticos, no ejecutes `ledger.py` y no escribas en `corpus/resultados/`.

El lote L0010 ya fue reservado en el ledger. Antes de procesar cada documento, verifica sus artefactos
...
El manifiesto reservado es `corpus/lotes/L0010.json` y contiene 15 documentos. La misión termina al completar
