...
**Orden canónico**: los JSON de producción deben conservar el orden de claves del esqueleto y el orden de las
listas jerárquicas del índice. `pipeline/validar_orden_json.py` bloquea claves reordenadas o campos opcionales
intercalados dentro de los bloques canónicos. No reescribe silenciosamente el artefacto.
...
| Campo                     | Regla                                                                                                                                                                                  |
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `num_muestra`             | En producción corresponde a `corpus_order`, el orden estable del corpus definitivo incluido en el manifiesto del lote; nunca es la posición local dentro del lote. Los pilotos históricos conservan su numeración original.                         |
...
- `ambiguedad_pendiente_validacion` es `null` cuando la decisión ya fue adjudicada o solo existe una alternativa descartada. Si contiene texto, primaria y secundaria llevan certeza `Baja` y debe existir una decisión humana realmente pendiente; el validador exige esta invariancia desde el congelamiento pre-batch v1.
